...
void ResBoundary::findCornerCells() {
  //GENERAL
...
void ResBoundary::printCornerXYZ(std::string str_out, Eigen::Vector3d vector_xyz) {
 ...
 */
QList<int> ResBoundary::getIndicesOfEdgeCells() {
...
bool ResBoundary::CaseSatisfiesConstraint(Case *c) {
...
void ResBoundary::SnapCaseToConstraints(Case *c) {
...
QList<int> ResBoundary::getListOfCellIndices() {
...
Eigen::VectorXd ResBoundary::GetLowerBounds(QList<QUuid> id_vector) const {
...
Eigen::VectorXd ResBoundary::GetUpperBounds(QList<QUuid> id_vector) const {
  auto cell_min = grid_->GetCell(imin_, jmin_, kmin_);
  auto cell_max = grid_->GetCell(imax_, jmax_, kmax_);
...
void ResBoundary::InitializeNormalizer(QList<Case *> cases) {
  normalizer_.set_max(1.0L);
  normalizer_.set_midpoint(0.0L);
  normalizer_.set_steepness(1.0L);
}
double ResBoundary::Penalty(Case *c) {
  if (CaseSatisfiesConstraint(c))
    ...
    if (VERB_OPT >= 2) Printer::ext_info("Reservoir boundary penalty (infty) for case " + c->id().toString().toStdString(),
                                         "Optimization", "ResBoundary");
    return INFINITY;
  }
}
long double ResBoundary::PenaltyNormalized(Case *c) {
  if (CaseSatisfiesConstraint(c))
    return 0.0L;
  else {
    if (VERB_OPT >= 2) Printer::ext_info("Reservoir boundary penalty (infty) for case " + c->id().toString().toStdString(),
