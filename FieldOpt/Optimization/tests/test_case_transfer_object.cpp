...
TEST_F(CaseTransferObjectTest, SerializationAndDeserializationAndGeneratedCase) {
  ...
  for (int ii=0; ii < test_case_3_4b3i3r_->integer_variables().size(); ++ii) {
    EXPECT_EQ(test_case_3_4b3i3r_->integer_variables().value(ii).second, c->integer_variables().value(ii).second);
  }

  for (int ii=0; ii < test_case_3_4b3i3r_->binary_variables().size(); ++ii) {
    EXPECT_EQ(test_case_3_4b3i3r_->binary_variables().value(ii).second, c->binary_variables().value(ii).second);
  }

  for (int ii=0; ii < test_case_3_4b3i3r_->real_variables().size(); ++ii) {
    EXPECT_FLOAT_EQ(test_case_3_4b3i3r_->real_variables().value(ii).second,    c->real_variables().value(ii).second);
  }
}
}
