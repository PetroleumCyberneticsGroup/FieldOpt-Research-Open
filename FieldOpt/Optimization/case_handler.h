...
class CaseHandler
{
 public:
  ...
  //!< Call the default constructor and add the base case
  //!< to list of evaluated cases.
  CaseHandler(Case *base_case);

  /*!
   * \brief AddNewCase
   * Add a new non-evaluated case to the queue.
   */
  void AddNewCase(Case *c);

  /*!
   * \brief AddNewCases
   * Add any number of non-evaluated cases to the queue.
   */
  void AddNewCases(QList<Case *> cases);

  /*!
   * \brief GetNextCaseForEvaluation
   * Get the next case to be evaluated.
   *
   * The returned case is also added to the
   * list of cases currently being evaluated.
   */
   ...
   * that has been marked as evaluated since the last time
   * ClearRecentlyEvaluatedCases() was called.
   */
   ...
   * a significant point is reached by the optimizer, for
   * example at the end of an iteration.
   */
  void ClearRecentlyEvaluatedCases();

  /*!
   * \brief QueuedCases
   * Get list of cases currently queued to be evaluated.
   */
  QList<Case *> QueuedCases() const;

  /*!
   * \brief CasesBeingEvaluated
   * Get the list of cases currently being evaluated.
   * \return
   */
  QList<Case *> CasesBeingEvaluated() const;

  /*!
   * \brief EvaluatedCases
   * Get list of cases that have been marked as evaluated.
   */
 ...
 private:
  ...
  //!< List of keys for Cases currently being evaluated.
  QList<QUuid> evaluating_;

  //!< List of keys for Cases that have already been evaluated.
  QList<QUuid> evaluated_;

  //!< List of keys that have recently been evaluated.
  QList<QUuid> evaluated_recently_;
