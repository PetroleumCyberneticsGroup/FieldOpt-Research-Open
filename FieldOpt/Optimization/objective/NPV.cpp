...
double NPV::value() const {
  try {
    ...
    for (int k = 0; k < components_->size(); ++k) {
      if (components_->at(k)->is_json_component) {
        continue;
      }
    ...
    for (int i = 0; i < components_->size(); ++i) {
      if (components_->at(i)->is_json_component) {
        continue;
      }
      if (components_->at(i)->usediscountfactor) {
      ...
      } else if (!components_->at(i)->usediscountfactor) {
    ...
    error("Failed to compute NPV. Returning 0.0");
    return 0.0;
  }
}
