...
/*!
* @brief This class implements the "Covariance Matrix
 * Adaption Evolutionary Strategy" (CMA-ES) algorithm.
*
* It samples the space using a multivariate normal distribution
* in each real-valued dimension. For each step the variables in
* the population is perturbed to achieve a zero mean, based on
* the covariance matrix. For each step the coverance matrix is
* based on the new distribution around the new mean.
*
* The implementation is based on the description found at:
* Hansen, N. (2006), "The CMA evolution strategy: a comparing
* review", Towards a new evolutionary computation. Advances
