...
/*!
 * @brief This class is an implementation of Efficient
 * Global Optimization (EGO), i.e., Bayesian Optimization
 * using Gaussian Process models applied to derivative-
 * free optimization.
 *
 * \todo Hyperparameter optimization: after N cases,
 * optimize the GP hyperparameters.
 *
 * \todo Convergence criterion: total squared error in model.
 *
 * \todo Convergence criterion: Combination of highest
 * expected value ans total squared uncertainty?
 */
class EGO : public Optimizer {
 ...
 private:
  ...
  //!< Number of random cases to be generated initially.
  int n_initial_guesses_;

  //!< The gaussian process to be used throughout the optimization run.
  libgp::GaussianProcess *gp_;

  //!< Acquisition function to be used throughout the optimization run.
  BayesianOptimization::AcquisitionFunction af_;

  //!< Aquisition function optimizer to be used throughout the optimization run.
  BayesianOptimization::AFOptimizers::AFPSO af_opt_;
