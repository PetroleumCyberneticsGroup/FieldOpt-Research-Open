...
EGO(Settings::Optimizer *settings,
  ...
  if (penalize_) {
    ...
    if (vp_.vOPT >= 1) {
      im_ = "Penalized base case. Original value: " + num2str(org_ofv);
      im_ += "; Penalized value: " + num2str(pen_ofv);
  ...
  if (settings->parameters().ego_init_sampling_method == "Random") {
    ...
    wm_ = "Only the Random and Uniform sampling methods are implemented.";
...
void EGO::handleEvaluatedCase(Case *c) {
  gp_->add_pattern(c->GetRealVarVector().data(),
                   normalizer_ofv_.normalize(c->objf_value()));
  if (isImprovement(c)) {
    updateTentativeBestCase(c);
    im_ = "Found new tentative best case: " + num2str(c->objf_value());
    ext_info(im_, md_, cl_, vp_.lnw);
  }
}
...
void EGO::iterate() {
  ...
  if (vp_.vOPT >= 3) {
    im_ = "Optimizing Gaussian Process kernel hyperparameters ... ";
    ext_info(im_, md_, cl_, vp_.lnw);
