...
class Optimizer : public Loggable
{
 ...
 public:
  ...
  enum TerminationCondition : int { NOT_FINISHED=0,
    MAX_EVALS_REACHED=1,
    MIN_STEP_LENGTH_REACHED=2,
    MAX_ITERS_REACHED=3,
    SUFFICIENT_IMPROVEMENT=4,
    OPT_CRITERIA_REACHED=5,
    DFTR_MIN_RADIUS_REACHED=6,
    DFTR_CRIT_NORM_1ST_ORD_LT_TOLF=7,
    DFTR_CRIT_NORM_2ND_ORD_LT_TOLF=8, // placeholder
    DFTR_MAX_NUM_RHO_INF_MET=9
  };
  ...
  //!< Tolerance for sufficient improvement in terms of objective  
  void setSufficientImprovementTolerance(double improvement_tol);

  //!< Set minimum allowable model deviation to perform optimization  
  void setMinModelDeviation(double model_tol); 
 ...
 protected:
  ...
  //!< Base case objective function value
  double base_case_obj_;

  //!< Tolerance for sufficient improvement in the objective
  double sufficient_improvement_tol_;

  //!< Minimum model deviation to perform optimization  
  double min_model_deviation_tol_;
  
  // Optimization mode, i.e. whether the objective
