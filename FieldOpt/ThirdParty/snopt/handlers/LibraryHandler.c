...
#else
#define mysnprintf snprintf
#error "Do not have function for save printing "
"into a C-string (snprintf or _snprintf)"
#endif
#endif
...
soHandle_t LSL_loadLib(const char *libName,
  ...
  mysnprintf(msgBuf, msgLen,
      ...
      "shared libraries on this operating system");
  return h;
#else
...
# ifdef HAVE_WINDOWS_H
  ...
  if (NULL == h) {
    mysnprintf(msgBuf, msgLen,
        "Windows error while loading dynamic"
        "library %s, error = %d.\n");
  }
# else
  ...
  if (NULL == h) {
    ...
    printf("\x1b[31mShared library loader error"
           "[dlopen()]: %s.\n\x1b[0m", msgBuf);
    msgBuf[msgLen-1]=0;
  }
# endif
...
// workaround for the problem that dlsym returns an object
// instead of a function pointer. (However, Windows also
// needs special care.)
//
// The method does six attempts to load the symbol. Next
// to its given name, it also tries variations of lower
// case and upper case form and with an extra underscore.
//
// @param h Handle of dynamicall linkes library.
// @param symName Name of the symbol to load.
// @param msgBuf Buffer for error msg, assumed to be NOT NULL!
...
symtype LSL_loadSym (soHandle_t h, const char *symName,
  ...
  for (trip = 1;  trip <= 6;  trip++) {
    ...
    if (NULL != s) {
      ...
      mysnprintf(msgBuf, msgLen,
          "Cannot find symbol %s in dynamic library, "
          "error = %d. ", symName, GetLastError());
    }
#else
# ifdef HAVE_DLFCN_H
    s = dlsym (h, tripSym);
    // we have only one chance; a successive
    // call to dlerror() returns NULL
    err = dlerror();
    if (err) {
