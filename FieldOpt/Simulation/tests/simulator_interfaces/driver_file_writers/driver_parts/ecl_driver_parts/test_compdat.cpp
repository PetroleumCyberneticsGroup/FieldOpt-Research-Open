...
class DriverPartCompdatTest :
  public ::testing::Test, public TestResources::TestResourceModel {
 protected:
  DriverPartCompdatTest(){
    cout << "model_->wells()->size(): " << model_->wells()->size() << endl;
    for (auto w : *model_->wells()) {
      cout << "w: " << w->name().toStdString() << endl;
    }
