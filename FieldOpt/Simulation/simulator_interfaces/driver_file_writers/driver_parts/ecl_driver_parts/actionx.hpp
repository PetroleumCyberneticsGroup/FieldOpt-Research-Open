...
/*!
 * @brief This file implements functions needed for the
 * ACTIONX keyword for the ECLIPSE 100 reservoir simulator.
 *
 * The ACTIONX keyword may be used to "store" schedule actions
 * for later use, when some condition is met. E.g. it may be
 * used to alter valve sizes at certain times without having
 * to place the keyword in the chronologically correct place
 * in the schedule.
 *
 ...
 *
 * ACTIONX
 * --  Action name  # allowed repeats  Min interval b/n repeats
 *     ACT_NAME            1                     0   /
...
enum ACTX_LHQuantity { Day, Month, Year };
enum ACTX_Operator { EQ, GT, LT, GE, LE, AND };

std::map<ACTX_LHQuantity, std::string> LHQuantityString = {
  std::pair<ACTX_LHQuantity, std::string> {Day, "DAY"},
  std::pair<ACTX_LHQuantity, std::string> {Month, "MNTH"},
 ...
 */
inline std::string ACTIONX(std::string name,
                           ACTX_LHQuantity lhq,
                           ACTX_Operator op,
                           double rhq,
                           std::string actions) {
...
inline std::string ACTIONX(std::string name,
                           ACTX_LHQuantity lhq_day,
                           ACTX_Operator op_day,
                           ACTX_LHQuantity lhq_mnth,
                           ACTX_Operator op_mnth,
                           ACTX_LHQuantity lhq_year,
                           ACTX_Operator op_year,
                           time_DMY rhq,
                           std::string actions) {
  std::stringstream actionx;
  actionx << "ACTIONX" << std::endl;
  actionx << "    " << name << "  1  0  /" << std::endl;
  actionx << "    " << LHQuantityString[lhq_day]  << "  " << OperatorString[op_day]  << "  " << rhq.day_str << " AND /" << std::endl;
  actionx << "    " << LHQuantityString[lhq_mnth] << "  " << OperatorString[op_mnth] << "  " << rhq.month_str << " AND /" << std::endl;
