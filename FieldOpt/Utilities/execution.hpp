...
inline QString Exec(QString directory, QStringList commands,
                    bool verbose=false, Settings::VerbParams vp={}) {
  if (!Utilities::FileHandling::DirExists(directory, vp))
...
inline void ExecShellScript(QString script_path, QStringList args,
  ...
  if (!Utilities::FileHandling::FileExists(script_path, vp))
...
/*!
 * @brief ExecShellScriptTimeout execututes a shell script
 * with the given set of parameters, and terminates the
 * process after a set time has passed if it has not
 * returned by then.
 *
 * \todo In this function, in the else block, we can also,
 * at a later stage, monitor the output files of a simulation
 * and use them to decide whether we should abort.
 *
 * @param script_path Absolute path to the shell script.
 * @param args Arguments to be passed to the script.
 ...
 */
inline bool ExecShellScriptTimeout(QString script_path,
                                   QStringList args,
                                   int timeout, Settings::VerbParams vp) {
  if (!Utilities::FileHandling::FileExists(script_path, vp))
    throw std::runtime_error("File not found: " + script_path.toStdString());
  ...
  pid  = helpers::fork_child(script_path, args, vp);
  to.tv_sec = timeout;
  to.tv_nsec = 0;
